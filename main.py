...
APP_DESCRIPTION = '<i>Используемая модель: ' \
                  '<a href="https://huggingface.co/speechbrain/lang-id-voxlingua107-ecapa/tree/main" ' \
                  'target="_blank">Spoken Language Identification Model</a></i>'
APP_REP = '<i> <a href="https://github.com/Dk-A-r/SoundML" target="_blank">Репозиторий проекта</a></i>'


def load_audio():
    """Создание формы для загрузки аудио"""
    uploaded_file = st.file_uploader(label='Загрузите аудиофайл для распознавания')
    if uploaded_file:
